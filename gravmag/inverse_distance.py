...
def grad(data_points, source_points, SEDM, components=['x', 'y', 'z'],
         check_input=True):
    '''
    ...
    if check_input is True:
        ...
        _, _indices = np.unique(np.asarray(components, dtype=str),
                                return_index=True)
...
def grad_tensor(data_points, source_points, SEDM,
    ...
    # define a dictionary for component indices
    component_indices = {'xx':(0,0), 'xy':(0,1), 'xz':(0,2),
