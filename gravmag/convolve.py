...
def general_BTTB(num_blocks, columns_blocks, rows_blocks=None):
    ...
    assert (isinstance(num_blocks, int)) and (num_blocks > 1), 'num_blocks \
    ...
    'the number of rows in "columns_blocks" must be equal to \
    "num_blocks" or equal to (2 * "num_blocks") - 1'

    block_size = columns_blocks.shape[1]
    ind_col_blocks, ind_row_blocks = np.ogrid[0:block_size, block_size-1:-1:-1]
    ...
    if rows_blocks is None:
        ...
        if (columns_blocks.shape[0] == num_blocks):
            ...
            for column in columns_blocks:
                blocks.append(np.concatenate((column[-1:0:-1],
                                              column))[ind_blocks])
            concatenated_blocks = np.concatenate((np.stack(blocks)[-1:0:-1],
                                                  np.stack(blocks)))
        ...
        if (columns_blocks.shape[0] == (2*num_blocks - 1)):
            ...
            for column in columns_blocks[:num_blocks]:
                blocks_1j.append(np.concatenate((column[-1:0:-1],
                                                 column))[ind_blocks])
            blocks_i1 = []
            for column in columns_blocks[num_blocks:]:
                blocks_i1.append(np.concatenate((column[-1:0:-1],
                                                 column))[ind_blocks])
            concatenated_blocks = np.concatenate((np.stack(blocks_i1)[::-1],
                                                  np.stack(blocks_1j)))

    else:
        rows_blocks = np.asarray(rows_blocks)
        assert (columns_blocks.shape[0] == rows_blocks.shape[0]), 'the number \
        of rows in "rows_and" and columns_blocks" must be equal to each other'
        assert (columns_blocks.shape[1] == (rows_blocks.shape[1]+1)), 'the \
        number of column in "columns_blocks must be equal that in \
        "rows_blocks" + 1'
        ...
        if (columns_blocks.shape[0] == num_blocks):
            ...
            for (column, row) in zip(columns_blocks, rows_blocks):
                blocks.append(np.concatenate((row[::-1],
                                              column))[ind_blocks])
            concatenated_blocks = np.concatenate((np.stack(blocks)[-1:0:-1],
                                                  np.stack(blocks)))
        ...
        if (columns_blocks.shape[0] == (2*num_blocks - 1)):
            blocks_1j = []
            for (column, row) in zip(columns_blocks[:num_blocks],
                                     rows_blocks[:num_blocks]):
                blocks_1j.append(np.concatenate((row[::-1],
                                                 column))[ind_blocks])
            blocks_i1 = []
            for (column, row) in zip(columns_blocks[num_blocks:],
                                     rows_blocks[num_blocks:]):
                blocks_i1.append(np.concatenate((row[::-1],
                                                 column))[ind_blocks])
            concatenated_blocks = np.concatenate((np.stack(blocks_i1)[::-1],
                                                  np.stack(blocks_1j)))
...
def C_from_T(T_column, T_row=None):
    ...
    if T_row is None:
        ...
        assert T_column.size == (T_row.size + 1), 'T_column size must be equal \
        to T_row size + 1'
...
def BCCB_from_BTTB(num_blocks, columns_blocks, rows_blocks=None):
    ...
    assert columns_blocks.ndim == 2, 'columns_blocks must be a matrix'
    assert (columns_blocks.shape[0] == num_blocks) or \
    (columns_blocks.shape[0] == (2*num_blocks - 1)), 'the number of rows \
    in "columns_blocks" must be equal to "num_blocks" or equal to \
    (2 * "num_blocks") - 1'
    ...
    if rows_blocks is None:
        ...
        if (columns_blocks.shape[0] == num_blocks):
            C_blocks = np.concatenate((np.stack(nonull_blocks),
        ...
        if (columns_blocks.shape[0] == (2*num_blocks - 1)):
            C_blocks = np.concatenate((np.stack(nonull_blocks[:num_blocks]),
                                       np.zeros((1, C_block_size, C_block_size)),
