...
def test_safe_atan2_numba():
    "Test the safe_atan2 function"
    # Test safe_atan2 for one point per quadrant
    # First quadrant
    x, y = 1, 1
    aae(utils.safe_atan2_numba(y, x), np.pi / 4, decimal=15)
    # Second quadrant
    x, y = -1, 1
    aae(utils.safe_atan2_numba(y, x), -np.pi / 4, decimal=15)
    # Third quadrant
